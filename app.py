...
def pikpak_poll_download(file_id, account, tokens, timeout=120, filename=None, file_hash=None):
    """Poll until download completes, with recovery for 404s."""
    print(f"PIKPAK [{SERVER_ID}]: Polling download status for {file_id} (hash: {file_hash})", flush=True)
    ...
    poll_interval = 5
    last_recovery_time = 0 # Start recovery check on first 404
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            # We need fresh tokens inside the loop
            access_token = ensure_logged_in(account)["access_token"]
            ...
            if response.status_code == 404 or "file_not_found" in response.text:
                ...
                print(f"PIKPAK [{SERVER_ID}]: File not ready yet (404), waiting... ({int(elapsed_time)}s elapsed)", flush=True)

                # Trigger recovery search every 30s
                if time.time() - last_recovery_time > RECOVERY_INTERVAL:
                    last_recovery_time = time.time()
                    print(f"PIKPAK [{SERVER_ID}]: Triggering Recovery Search...", flush=True)
                    ...
                    try:
                        ...
                        if file_hash:
                            for file in files:
                                try:
                                    # Get full file info to check hash
                                    info_tokens = ensure_logged_in(account)
                                    file_info = pikpak_get_file_info(file.get('id'), account, info_tokens)
                                    params_url = file_info.get('params', {}).get('url', '')
                                    
                                    if file_hash in params_url.upper():
                                        print(f"PIKPAK [{SERVER_ID}]: Recovery: Found match by HASH: {file_hash}", flush=True)
                                    ...
                                    print(f"PIKPAK [{SERVER_ID}]: Recovery: Error checking file info for hash: {e}", flush=True)

                        # FALLBACK TO NAME MATCH if no hash match found
                        ...
                        if found_match:
                            continue # Restart polling loop with new file_id

                    except Exception as e:
                        print(f"PIKPAK [{SERVER_ID}]: Recovery Search failed: {e}", flush=True)

                time.sleep(poll_interval)
                continue # Continue to next poll attempt
            ...
            # If not complete and no useful info, log for debugging
            if not phase and not progress:
...
def extract_hash(magnet_link):
    """Extract BTIH hash from magnet link and normalize it."""
    match = re.search(r'xt=urn:btih:([a-zA-Z0-9]+)', magnet_link, re.IGNORECASE)
    if match:
        return match.group(1).upper()
    return None

def check_duplicate_by_hash(magnet_link, account, tokens):
    """Check for duplicate file by BTIH hash."""
    input_hash = extract_hash(magnet_link)
    if not input_hash:
        print(f"PIKPAK [{SERVER_ID}]: No hash found in magnet link. Skipping check.", flush=True)
        return None
    ...
    try:
        ...
        for file in files:
            ...
            try:
                # We need fresh tokens for each call inside the loop potentially
                fresh_tokens = ensure_logged_in(account)
                file_info = pikpak_get_file_info(file_id, account, fresh_tokens)
                
                # The hash is often in params.url for magnet links
                params_url = file_info.get('params', {}).get('url', '')
                ...
                if is_match:
                    print(f"PIKPAK [{SERVER_ID}]: ✅ Hash match FOUND for file: {file_name} ({file_id})", flush=True)
                    return file 

            except Exception as e:
                print(f"PIKPAK [{SERVER_ID}]: Could not get info for file {file_name} ({file_id}): {e}", flush=True)
                continue

    except Exception as e:
        print(f"PIKPAK [{SERVER_ID}]: Error during hash duplicate check: {e}", flush=True)

    print(f"PIKPAK [{SERVER_ID}]: No duplicate found for hash: {input_hash}", flush=True)
    return None
    ...
    while attempt < max_total_retries:
        ...
        try:
            with MAGNET_ADD_LOCK:
                ...
                # 2. Check for duplicate by hash (NEW)
                existing_file = check_duplicate_by_hash(magnet, account, tokens)
                if existing_file:
                    print(f"PIKPAK [{SERVER_ID}]: ✅ Found existing file by hash: {existing_file.get('name')}", flush=True)
                    
                    try:
                        download_url = pikpak_get_download_link(existing_file['id'], account, tokens)
                        file_size = int(existing_file.get('size', 0))
                        
                        detected_quality = detect_quality(user_quality, magnet, file_size)
                        
                        return jsonify({
                            "result": True,
                            "folder_id": existing_file['id'],
                        ...
                        print(f"PIKPAK [{SERVER_ID}]: Could not process existing file found by hash (will re-download): {e}", flush=True)
                ...
                pikpak_poll_download(
                    ...
                    tokens=tokens, 
                    timeout=900,
                    filename=filename_from_magnet,
                    file_hash=hash_from_magnet
                )
